...
def download_subtitle(sub_id, download_link=None):
    ...
    try:
        ...
        # ── Extragere RAR — via addon vfs.rar (Kodi repo oficial) ───────────
        if is_rar or (not is_zip and not is_rar):
            rar_ok = False
            try:
                encoded  = urllib.parse.quote(archive, safe='')
                vfs_root = f"rar://{encoded}/"
                log(f"Incerc vfs.rar: {vfs_root}")
                _, files = xbmcvfs.listdir(vfs_root)
                if files:
                    chosen_name = pick_subtitle(files, player.getPlayingFile())
                    if chosen_name:
                        fh = xbmcvfs.File(vfs_root + chosen_name)
                        chosen_content = bytes(fh.readBytes())
                        fh.close()
                        if chosen_content:
                            log(f"RAR extras via vfs.rar: {chosen_name}")
                            rar_ok = True
            except Exception as e:
                log(f"vfs.rar eroare: {e}", xbmc.LOGWARNING)
